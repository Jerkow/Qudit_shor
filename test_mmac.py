...
class register():
    ...
    def coefs(self):
        prob = np.zeros(self.dimension**self.length,dtype=complex)
        for i in range(len(prob)):
            i_d = base10_to_d(i,self.dimension,self.length)
            productory = complex(1.0,0.0)
            for m in range(self.length):
                productory = productory*self.register[m].total[i_d[m]]
            prob[i] = productory
        self.coef = prob
...
q2 = qudit(4,amp=(1,0,0,0), phase=(0,0,0,0))


r1 = register([q1,q2])
print(r1.register[0].values)
r1.qft()
print('qft aplied')
